...
function queryAndSet2kkiLocation(mapId, prevMapId, prevLocations, setLocationFunc, forClient) {
  return new Promise((resolve, reject) => {
    let url = `https://explorer.yume.wiki/?action=getMapLocationNames&mapId=${mapId}`;
    if (prevMapId) {
...
function queryConnected2kkiLocationNames(locationName, connLocationNames) {
  return new Promise((resolve, _reject) => {
    const url = `https://explorer.yume.wiki/?action=getConnectedLocations&locationName=${locationName}&connLocationNames=${connLocationNames.join('&connLocationNames=')}`;
    const callback = response => {
      let ret = [];
      let errCode = null;
...
function queryAndSet2kkiMaps(locationNames) {
  return new Promise((resolve, _reject) => {
    ...
    const url = `https://explorer.yume.wiki/?action=getLocationMaps&locationNames=${massagedLocationNames.join('&locationNames=')}`;
    const callback = response => {
      let errCode = null;
